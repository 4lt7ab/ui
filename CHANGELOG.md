...
- Internal refactor — rebuild `Card` on `<Surface>`. `Card.tsx` now renders a `<Surface>` for the background/border/shadow/padding/radius concerns, with each variant mapped to a Surface prop bag (`default` → `level='solid' + border + shadow='sm'`, `flat` → `level='raised' + border`, `elevated` → `level='solid' + border + shadow='md'`). Card's `hover` / `glow` / `asChild` behavior stays at the Card level — hover/glow are specific to the interactive-card pattern and `asChild` merges onto Slot directly (so Surface's API stays unchanged for that concern). A shared `getSurfaceStyle()` helper exported from Surface keeps Card's asChild path and Surface's own render pixel-identical to the old Card output — all 10 Card tests (including the asChild suite) and the 8 LinkCard tests pass untouched. Surface gains a minimal internal `style?: CSSProperties` merge hook and a `dataAttributes?: Record<…>` forwarder so composites (Card) can stack a glow box-shadow and attach the `data-card-hover` / `data-card-glow` CSS selectors without widening the consumer-facing API. Guarantees Card and Surface stay visually in sync going forward (task `01KPH0VZFC86VTGFDGT1DT0Y7M`)
