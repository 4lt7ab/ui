...
- Fix ThemeSurface `global` prop using `getComputedStyle` to resolve `--color-surface-page`, which raced with ThemeProvider token application on fast theme switches; now sets `var(--color-surface-page)` directly for instant, correct updates
