...
- Add `Calendar.Root` + `useCalendarContext()` to `@4lt7ab/ui` — the state-owning primitive for the upcoming compound Calendar API. Root owns mode (`'single' | 'range'`), selection, min/max, a `disabledDate` predicate, and focused-date state (controlled or uncontrolled via `defaultFocusedDate` + `onFocusedDateChange`). Exposes the full API via context so downstream primitives (Header, Nav, Grid, Cell — landing in subsequent slices) can register against it. No visual change yet: existing `DatePicker` and `DateRangePicker` continue to render via their current internals. Part of the calendar-compound refactor sequence (tracking group `calendar-compound`)
