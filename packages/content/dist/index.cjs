...
`;function KJ(){return YJ[Math.random()*YJ.length|0]}var qQ=G.forwardRef(function({words:Q,holdMs:X=2000,scrambleTicks:z=4,tickMs:V=50,staggerMs:q=30},Z){GJ.useInjectStyles(UQ,ZQ);let U=G.useRef(null);G.useImperativeHandle(Z,()=>U.current);let H=G.useRef(0),w=G.useRef([]),v=G.useRef(null),A=G.useRef(null),O=G.useCallback(()=>{let W=U.current;if(!W)return;let B=W.innerHTML;W.style.width="";let Y=[];for(let L of Q)W.textContent=L,Y.push(Math.ceil(W.getBoundingClientRect().width));W.innerHTML=B||"",w.current=Y,W.style.width=Y[H.current]+"px"},[Q]),m=G.useCallback((W)=>{let B=U.current;if(!B)return;B.innerHTML="";for(let Y=0;Y<W.length;Y++){let L=document.createElement("span");L.className="alttab-char",L.textContent=W[Y],L.style.animationDelay=`${Y*q}ms`,B.appendChild(L)}},[q]),j=G.useCallback((W,B)=>{let Y=U.current;if(!Y)return;if(window.matchMedia("(prefers-reduced-motion: reduce)").matches){Y.textContent=W,B();return}Y.innerHTML="";let u=[];for(let N=0;N<W.length;N++){let D=document.createElement("span");D.className="alttab-char",D.style.animationDelay=`${N*q}ms`,D.textContent=W[N]===" "?" ":KJ(),Y.appendChild(D),u.push({span:D,final:W[N],ticks:0,maxTicks:z+N*2})}A.current=setInterval(()=>{let N=!0;for(let D of u)if(D.ticks>=D.maxTicks)D.span.textContent=D.final;else N=!1,D.ticks++,D.span.textContent=D.final===" "?" ":KJ();if(N)clearInterval(A.current),A.current=null,B()},V)},[z,V,q]);return G.useEffect(()=>{if(Q.length<2)return;O(),document.fonts.ready.then(O);let W=()=>O();document.fonts.addEventListener("loadingdone",W);let B=new MutationObserver(()=>requestAnimationFrame(O));B.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]}),H.current=0,m(Q[0]);function Y(){H.current=(H.current+1)%Q.length;let L=U.current;if(L)L.style.width=w.current[H.current]+"px";j(Q[H.current],()=>{v.current=setTimeout(Y,X)})}return v.current=setTimeout(Y,X),()=>{if(v.current)clearTimeout(v.current);if(A.current)clearInterval(A.current);document.fonts.removeEventListener("loadingdone",W),B.disconnect()}},[Q,X,O,m,j]),DJ.jsx("span",{ref:U,className:"alttab-thinking"})});
