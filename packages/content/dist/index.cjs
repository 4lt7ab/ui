...
`;function aJ({children:J}){let $=0,X=G.Children.map(J,(W)=>{if(!G.isValidElement(W))return W;let K=$%2===1;if($++,!K)return W;let Y=G.Children.map(W.props.children,(V)=>{if(!G.isValidElement(V))return V;let Z=V.props;return G.cloneElement(V,{style:{...Z.style,background:`color-mix(in srgb, var(--color-text) ${j}, transparent)`}})});return G.cloneElement(W,{},Y)});return z.jsx("tbody",{children:X})}var iJ="alttab-markdown-editable",nJ=`
  ...
  .alttab-md-editable-overlay:focus-visible {
    outline: ${F.semantic.focusRingWidth} solid ${F.semantic.focusRingColor};
    outline-offset: ${F.semantic.focusRingOffset};
  }
...
`,$J={display:"inline-flex",alignItems:"center",justifyContent:"center",gap:F.semantic.spaceSm,padding:`${F.semantic.spaceSm} ${F.semantic.spaceMd}`,fontFamily:F.semantic.fontSans,fontSize:F.semantic.fontSizeSm,fontWeight:F.semantic.fontWeightMedium,lineHeight:F.semantic.lineHeightTight,borderRadius:F.semantic.radiusMd,cursor:"pointer",transition:`background ${F.semantic.transitionBase}, border-color ${F.semantic.transitionBase}, opacity ${F.semantic.transitionBase}`},tJ={...$J,background:F.semantic.colorActionPrimary,color:F.semantic.colorTextInverse,border:"none"},eJ={...$J,background:F.semantic.colorActionSecondary,color:F.semantic.colorText,border:`${F.semantic.borderWidthDefault} solid ${F.semantic.colorBorder}`},n={pre:hJ,blockquote:sJ,tbody:aJ,h1:({children:J})=>z.jsx(v,{level:1,children:J}),h2:({children:J})=>z.jsx(v,{level:2,children:J}),h3:({children:J})=>z.jsx(v,{level:3,children:J}),h4:({children:J})=>z.jsx(v,{level:4,children:J}),h5:({children:J})=>z.jsx(v,{level:5,children:J}),h6:({children:J})=>z.jsx(v,{level:6,children:J})};function p({children:J,id:$,"data-testid":X,editable:W=!1,editing:K=!1,value:Y,onStartEdit:V,onEditChange:Z,onSave:H,onCancel:A,fieldLabel:B,rows:M=4,placeholder:T="Click to add content..."}){F.useInjectStyles(lJ,rJ),F.useInjectStyles(iJ,nJ);let[k,y]=G.useState(!1),U=G.useRef(null),q=J??"",Q=G.useCallback(()=>{if(navigator.clipboard.writeText(q),y(!0),U.current)clearTimeout(U.current);U.current=setTimeout(()=>y(!1),1500)},[q]),O=G.useCallback((N)=>{if(N.key==="Escape")N.preventDefault(),A?.();else if(N.key==="Enter"&&(N.metaKey||N.ctrlKey))N.preventDefault(),H?.()},[A,H]);if(W){if(K)return z.jsxs("div",{role:"group","aria-label":B,children:[z.jsx("textarea",{value:Y??"",onChange:(N)=>Z?.(N.target.value),onKeyDown:O,rows:M,"aria-label":B?`Edit ${B}`:"Edit content",style:{display:"block",width:"100%",boxSizing:"border-box",padding:F.semantic.spaceSm,fontFamily:F.semantic.fontMono,fontSize:F.semantic.fontSizeSm,lineHeight:F.semantic.lineHeightRelaxed,color:F.semantic.colorText,background:F.semantic.colorSurfacePage,border:`${F.semantic.borderWidthDefault} solid ${F.semantic.colorBorder}`,borderRadius:F.semantic.radiusMd,resize:"vertical",outline:"none"}}),z.jsxs("div",{style:{display:"flex",gap:F.semantic.spaceSm,marginTop:F.semantic.spaceSm},children:[z.jsx("button",{type:"button",onClick:H,style:tJ,children:"Save"}),z.jsx("button",{type:"button",onClick:A,style:eJ,children:"Cancel"})]})]});if(!q)return z.jsx("div",{className:"alttab-md-editable-empty",role:"button",tabIndex:0,"aria-label":B?`Add ${B}`:"Add content",onClick:V,onKeyDown:(N)=>{if(N.key==="Enter"||N.key===" ")N.preventDefault(),V?.()},children:T});return z.jsxs("div",{className:"alttab-md-editable-display",children:[z.jsx("button",{type:"button",className:"alttab-md-editable-overlay","aria-label":B?`Edit ${B}`:"Edit content",onClick:V}),z.jsx("div",{className:"alttab-markdown",id:$,"data-testid":X,children:z.jsx(x.default,{remarkPlugins:[f.default],components:n,children:q})})]})}return z.jsxs("div",{className:"alttab-markdown",id:$,"data-testid":X,children:[z.jsx("button",{type:"button",className:"alttab-md-copy-source",onClick:Q,"aria-label":"Copy markdown source",title:"Copy markdown",...k?{"data-copied":""}:{},children:k?z.jsx(JJ,{}):z.jsx(e,{})}),z.jsx(x.default,{remarkPlugins:[f.default],components:n,children:q})]})}var zJ=require("react"),FJ=require("../../core/dist/index.cjs"),L=require("react/jsx-runtime"),J1="alttab-epigraph",$1=`
  .alttab-epigraph {
    border: none;
    border-block: var(--border-width-default) solid var(--color-border);
...
`;function HJ(){return qJ[Math.random()*qJ.length|0]}var U1=D.forwardRef(function({words:$,holdMs:X=2000,scrambleTicks:W=4,tickMs:K=50,staggerMs:Y=30},V){DJ.useInjectStyles(V1,G1);let Z=D.useRef(null);D.useImperativeHandle(V,()=>Z.current);let H=D.useRef(0),A=D.useRef([]),B=D.useRef(null),M=D.useRef(null),T=D.useCallback(()=>{let U=Z.current;if(!U)return;let q=U.innerHTML;U.style.width="";let Q=[];for(let O of $)U.textContent=O,Q.push(Math.ceil(U.getBoundingClientRect().width));U.innerHTML=q||"",A.current=Q,U.style.width=Q[H.current]+"px"},[$]),k=D.useCallback((U)=>{let q=Z.current;if(!q)return;q.innerHTML="";for(let Q=0;Q<U.length;Q++){let O=document.createElement("span");O.className="alttab-char",O.textContent=U[Q],O.style.animationDelay=`${Q*Y}ms`,q.appendChild(O)}},[Y]),y=D.useCallback((U,q)=>{let Q=Z.current;if(!Q)return;if(window.matchMedia("(prefers-reduced-motion: reduce)").matches){Q.textContent=U,q();return}Q.innerHTML="";let N=[];for(let _=0;_<U.length;_++){let P=document.createElement("span");P.className="alttab-char",P.style.animationDelay=`${_*Y}ms`,P.textContent=U[_]===" "?" ":HJ(),Q.appendChild(P),N.push({span:P,final:U[_],ticks:0,maxTicks:W+_*2})}M.current=setInterval(()=>{let _=!0;for(let P of N)if(P.ticks>=P.maxTicks)P.span.textContent=P.final;else _=!1,P.ticks++,P.span.textContent=P.final===" "?" ":HJ();if(_)clearInterval(M.current),M.current=null,q()},K)},[W,K,Y]);return D.useEffect(()=>{if($.length<2)return;T(),document.fonts.ready.then(T);let U=()=>T();document.fonts.addEventListener("loadingdone",U);let q=new MutationObserver(()=>requestAnimationFrame(T));q.observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]}),H.current=0,k($[0]);function Q(){H.current=(H.current+1)%$.length;let O=Z.current;if(O)O.style.width=A.current[H.current]+"px";y($[H.current],()=>{B.current=setTimeout(Q,X)})}return B.current=setTimeout(Q,X),()=>{if(B.current)clearTimeout(B.current);if(M.current)clearInterval(M.current);document.fonts.removeEventListener("loadingdone",U),q.disconnect()}},[$,X,T,k,y]),NJ.jsx("span",{ref:Z,className:"alttab-thinking"})});
